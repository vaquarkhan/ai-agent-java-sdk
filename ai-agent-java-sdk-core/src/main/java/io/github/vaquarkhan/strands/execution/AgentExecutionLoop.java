...
import io.github.vaquarkhan.strands.approval.ApprovalDecision;
import io.github.vaquarkhan.strands.approval.ApprovalManager;
import io.github.vaquarkhan.strands.conversation.ConversationManager;
...
import io.github.vaquarkhan.strands.hook.ToolCallPolicyDecision;
...
public class AgentExecutionLoop {
    ...
    public AgentExecutionLoop(LoopModelClient modelClient, int maxIterations, String agentId) {
    ...
    public AgentLoopResult run(
        ...
        for (int iteration = 1; iteration <= maxIterations; iteration++) {
            ...
            try {
                ...
                dispatchHook(new AgentHookEvent.BeforeModelCall(iteration, prompt));

                ModelTurnResponse response = callModelWithRetry(prompt, toolRegistry);

                // Dispatch after-model-call hook
                dispatchHook(new AgentHookEvent.AfterModelCall(iteration, response));
                ...
                if (response.hasMultipleToolCalls()) {
                    ...
                    for (ToolCallRequest request : requests) {
                        ...
                        if (!eval.allowed()) {
                            ...
                            dispatchHook(new AgentHookEvent.AfterToolCall(iteration, request.toolName(), deniedResult));
                        ...
                        dispatchHook(new AgentHookEvent.BeforeToolCall(
                    ...
                    for (int i = 0; i < results.size(); i++) {
                        ...
                        dispatchHook(new AgentHookEvent.AfterToolCall(iteration, request.toolName(), toolResult));
                        trace = observability.recordIteration(trace, iteration, request.toolName(), request.arguments(),
                ...
                if (response.hasToolCall()) {
                    ...
                    if (request == null) {
                        ...
                        dispatchHook(new AgentHookEvent.AfterToolCall(
                    ...
                    dispatchHook(new AgentHookEvent.BeforeToolCall(iteration, request.toolName(), request.arguments()));
                ...
                return new AgentLoopResult(partialContent, trace, TerminationReason.MODEL_COMPLETION, iteration, elapsed);
        ...
        return new AgentLoopResult(partialContent, trace, TerminationReason.MAX_ITERATIONS_REACHED, maxIterations, elapsed);
    }
    ...
    private void streamLoop(
        ...
        try {
            ...
            sink.error(new AgentExecutionException("Streaming execution failed", exception, 0, trace));
        }
    }
            ...
            try {
                ...
                if (attempt < maxRetries) {
                    ...
                    try {
                        ...
                        throw new AgentExecutionException("Retry interrupted", ie, 0, null);
                    }
                }
            }
        }
     ...
     */
    private List<ExecutionMessage> applyConversationManager(List<ExecutionMessage> history, AgentExecutionContext context) {
    ...
    private ToolCallRequest requireApproval(int iteration, ToolCallRequest request, AgentExecutionContext context) {
