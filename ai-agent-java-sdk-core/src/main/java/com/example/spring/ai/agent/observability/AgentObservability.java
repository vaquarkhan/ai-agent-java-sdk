package com.example.spring.ai.agent.observability;
...
import com.example.spring.ai.agent.model.IterationRecord;
import com.example.spring.ai.agent.model.IterationType;
...
public class AgentObservability {
    ...
    private final AiAgentProperties.Security security;

    public AgentObservability(
            MeterRegistry meterRegistry,
            ObservationRegistry observationRegistry,
            AiAgentProperties.Security security) {
        this.meterRegistry = meterRegistry;
        this.observationRegistry = observationRegistry == null ? ObservationRegistry.NOOP : observationRegistry;
        this.security = security;
    }

    public ReasoningTrace startTrace(String agentId, AgentExecutionContext context) {
    ...
    public ReasoningTrace recordIteration(
            ...
            String toolOutput,
            Duration elapsed) {
        Observation iterationObservation = Observation.start("ai.agent.iteration", observationRegistry)
        ...
        try (Observation.Scope ignored = iterationObservation.openScope()) {
            ...
            updated.add(record);
            logger.debug("ai.agent iteration={} tool={} summary={}", iterationNumber, toolName, iterationType);
            logger.info("ai.agent iteration={} tool={} elapsedMs={}",
    ...
    public void recordMetrics(
            ReasoningTrace trace, TerminationReason reason, int totalIterations, Duration totalElapsed) {
        DistributionSummary.builder("ai.agent.iteration.count").register(meterRegistry).record(totalIterations);
        Timer.builder("ai.agent.loop.duration").register(meterRegistry).record(totalElapsed);
        if (reason == TerminationReason.MAX_ITERATIONS_REACHED) {
            Counter.builder("ai.agent.loop.max_iteration_termination").register(meterRegistry).increment();
        }
        trace.iterations().stream()
                .filter(record -> record.toolName() != null)
                .forEach(record -> Counter.builder("ai.agent.tool.invocations")
