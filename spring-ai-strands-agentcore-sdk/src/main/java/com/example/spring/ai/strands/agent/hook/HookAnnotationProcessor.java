 ...
 *
 * <p>Methods must accept exactly one parameter assignable from {@link StrandsHookEvent}.
...
public class HookAnnotationProcessor {
    ...
    public int process(Object bean) {
        ...
        for (Method method : methods) {
            ...
            Class<? extends StrandsHookEvent> eventType = annotation.value();

            // Validate method signature: must have exactly one parameter assignable from StrandsHookEvent
            Class<?>[] paramTypes = method.getParameterTypes();
            if (paramTypes.length != 1 || !StrandsHookEvent.class.isAssignableFrom(paramTypes[0])) {
                log.warn("Skipping @OnHook method '{}' on {}: must accept exactly one StrandsHookEvent parameter",
