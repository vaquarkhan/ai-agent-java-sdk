...
The [Strands Agents SDK](https://github.com/strands-agents/sdk-python) ships first-class support for **Python** and **TypeScript**. This repository's **spring-ai-strands-agentcore-sdk** implements the same **model-driven** pattern on the JVM: model plus tools in a loop until a final answer, integrated with **Spring Boot** and **Spring AI**.
...
| **Agent entry point** | `Agent()` | `StrandsAgent` (auto-configured bean) |
| **Execution loop** | `event_loop_cycle` (recursive) | `StrandsExecutionLoop.run()` (iterative) |
...
| **System prompt** | `Agent(system_prompt="...")` | `strands.agent.system-prompt` or `system-prompt-resource` |
...
| **Session context** | Python context objects | `StrandsExecutionContext` (session ID, user ID, headers) |
...
| **Configuration** | Code + environment variables | `application.yml` under `strands.agent.*` |
...
| Topic | Python Strands | Java / Spring AI (this SDK) |
|-------|---------------|----------------------------|
| **Hook system** | `HookRegistry` with `BeforeInvocationEvent`, `BeforeModelCallEvent`, etc. | `HookRegistry` with `StrandsHookEvent.BeforeInvocation`, `.BeforeModelCall`, etc. |
| **Hook annotation** | `@hook` decorator | `@OnHook(StrandsHookEvent.BeforeModelCall.class)` annotation |
| **Hook processor** | Auto-discovery in Plugin | `HookAnnotationProcessor.process(bean)` |
| **Plugin system** | `Plugin` base class with `init_agent()` | `StrandsPlugin` interface with `init(agent)` |
...
| Topic | Python Strands | Java / Spring AI (this SDK) |
|-------|---------------|----------------------------|
| **Agent state** | `AgentState` class | `ExecutionMessage` list + `StrandsExecutionContext` |
...
| **Retry logic** | Error handler with retry/backoff | `StrandsExecutionLoop` with `setRetryEnabled()`, `setMaxRetries()`, `setBackoffMillis()` |
...
| **Metrics** | OpenTelemetry metrics | Micrometer meters (`strands.iteration.count`, `strands.loop.duration`, etc.) |
| **Logging** | Python `logging` module | SLF4J (DEBUG per-iteration, INFO timing) |
| **Trace output** | In-memory traces on `AgentResult` | `ReasoningTrace` on `StrandsAgentResponse` |
...
| **Session isolation** | Not built-in | Immutable `StrandsExecutionContext`, file locking |
...
In Java, you build this by composing multiple `StrandsAgent` instances with a coordinator:
...
public class SwarmCoordinator {
    ...
    private final Map<String, StrandsAgent> agents;

    public SwarmCoordinator(
            @Qualifier("researcher") StrandsAgent researcher,
            @Qualifier("coder") StrandsAgent coder,
            @Qualifier("reviewer") StrandsAgent reviewer) {
        this.agents = Map.of(
    ...
    public String execute(String task) {
        ...
        while (handoffs < maxHandoffs) {
            StrandsAgent agent = agents.get(currentAgent);
            StrandsAgentResponse response = agent.execute(
                currentInput,
                StrandsExecutionContext.standalone("swarm-" + handoffs)
            );
...
In Java, you build this with a simple pipeline that chains `StrandsAgent` calls:
...
public class ResearchPipeline {
    ...
    private final StrandsAgent researcher;
    private final StrandsAgent analyst;
    private final StrandsAgent factChecker;
    private final StrandsAgent reportWriter;

    public String execute(String task) {
        // Step 1: Research
        String researchResult = researcher.execute(
            task,
            StrandsExecutionContext.standalone("research")
        ).content();
        ...
        CompletableFuture<String> analysisFuture = CompletableFuture.supplyAsync(() ->
            analyst.execute(
                ...
                StrandsExecutionContext.standalone("analysis")
            ).content()
        );
        ...
        CompletableFuture<String> factCheckFuture = CompletableFuture.supplyAsync(() ->
            factChecker.execute(
                ...
                StrandsExecutionContext.standalone("fact-check")
            ).content()
        );
        ...
        return reportWriter.execute(
            ...
            StrandsExecutionContext.standalone("report")
        ).content();
    }
}
```
...
```java
// application.yml
// strands.agent.system-prompt: "You are a research agent..."
...
public class ResearchAgent {
    ...
    public String research(String topic) {
        ...
        return agent.execute(
            ...
            StrandsExecutionContext.standalone("research-session")
        ).content();
    }
}
```
...
public class CodeReviewHooks {
    ...
    public void logToolCall(StrandsHookEvent event) {
    ...
    public void logCompletion(StrandsHookEvent event) {
...
StrandsAgentResponse response = agent.execute(
    ...
    StrandsExecutionContext.standalone("code-review")
);
```
...
public Map<String, String> support(@RequestBody SupportRequest req) {
    StrandsAgentResponse response = agent.execute(
        req.message(),
        StrandsExecutionContext.standalone(req.sessionId())
    );
    return Map.of(
...
@Component
public class SecurityPlugin implements StrandsPlugin {
    ...
    @OnHook(StrandsHookEvent.BeforeToolCall.class)
    public void auditToolCall(StrandsHookEvent event) {
        var e = (StrandsHookEvent.BeforeToolCall) event;
...
|---------|---------------|-----------------|
| Core execution loop | `strands.agent` + `strands.event_loop` | **This SDK** (`StrandsAgent`, `StrandsExecutionLoop`) |
...
| A2A protocol | `strands.agent.a2a_agent` | **spring-ai-a2a** library |
